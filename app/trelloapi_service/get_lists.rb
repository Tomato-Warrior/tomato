class GetLists
  ...
  def get_assigned_cards(member_id, board_name, list_name, api_key, token)
    url = "api.trello.com/1/search?query=member:#{member_id}%20board:#{board_name}%20list:#{list_name}%20sort:edited&card_fields=name,idList&cards_limit=1000&key=#{api_key}&token=#{token}".force_encoding('ASCII-8BIT')
    begin
